...
def test_search_with_no_arguments_should_return_all_files(conn, tmpfile, sample_filetag):
  files = tag.search(conn)
  assert len(files) == 1

def test_search_with_no_tags_should_return_matching_files(conn, tmpfile, sample_filetag):
  assert len(tag.search(conn, tags={"testtag": None})) == 1
  assert len(tag.search(conn, tags={"badtag": None})) == 0

def test_search_with_mime_type_should_return_matching_files(conn, tmpfile, sample_filetag):
  assert len(tag.search(conn, mime_types=["text/plain"])) == 1
  assert len(tag.search(conn, mime_types=["text/bad"])) == 0

def test_search_with_mime_type_should_support_wildcard_matching(conn, tmpfile, sample_filetag):
